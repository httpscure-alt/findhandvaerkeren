        ...
        <div className="bg-white">
            ...
            <section className="relative pt-20 pb-20 md:pt-32 md:pb-32 overflow-hidden bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
                    ...
                    <h1 className="text-4xl md:text-6xl font-extrabold text-[#1D1D1F] tracking-tight mb-6 max-w-4xl mx-auto leading-[1.1]">
                        {isDa
                            ? 'Få flere leads med Google Ads & SEO'
                            : 'Get more leads with Google Ads & SEO'}
                    </h1>
                    <p className="max-w-2xl mx-auto text-lg md:text-xl text-[#86868B] mb-10 leading-relaxed font-medium">
                        {isDa
                            ? 'Vi hjælper danske håndværkere med at dominere deres lokale marked. Fra første klik til færdig opgave.'
                            : 'We help Danish craftsmen dominate their local market. From the first click to the finished job.'}
                    </p>
                    ...
                    <div className="flex flex-col items-center justify-center gap-6">
                        ...
                        >
                            <TrendingUp size={20} />
                            {isDa ? 'Se hvordan vi vækster din virksomhed' : 'See how we grow your business'}
                        </button>
                    </div>
                </div>
            ...
            <section id="growth-services" className="py-24 bg-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 animate-in fade-in slide-in-from-bottom-8 duration-500">
                        {tiers[activeTab].map((tier) => (
                            ...
                            >
                                ...
                                >
                                    {isDa ? 'Start Vækst nu' : 'Start Growth now'}
                                    <ArrowRight size={16} />
                                </button>
                    ...
                    {/* NEW: CPL BENCHMARKS SECTION */}
                    <div className="mb-24">
                        <div className="text-center mb-12">
                            <h3 className="text-2xl md:text-3xl font-bold text-[#1D1D1F] mb-4">
                                {isDa ? 'Hvad koster et lead?' : 'What does a lead cost?'}
                            </h3>
                            <p className="text-[#86868B] font-medium">
                                {isDa ? 'Estimeret pris pr. lead baseret på det danske marked' : 'Estimated cost per lead based on the Danish market'}
                            </p>
                        </div>
                        
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                            {[
                                { cat: 'Tømrer', cpl: '150 - 300' },
                                { cat: 'VVS', cpl: '200 - 450' },
                                { cat: 'Elektriker', cpl: '180 - 400' },
                                { cat: 'Maler', cpl: '100 - 250' }
                            ].map((item, i) => (
                                <div key={i} className="bg-gray-50 p-6 rounded-3xl border border-gray-100 text-center">
                                    <p className="text-xs font-bold text-[#86868B] uppercase tracking-widest mb-2">{item.cat}</p>
                                    <p className="text-2xl font-black text-[#1D1D1F]">{item.cpl} <span className="text-xs">DKK</span></p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {activeTab === 'ads' && (
                        <div className="max-w-3xl mx-auto bg-gray-900 rounded-[2.5rem] p-8 md:p-12 text-center text-white shadow-2xl relative overflow-hidden">
                            <div className="relative z-10">
                                <h3 className="text-2xl font-bold mb-4">{isDa ? 'Større budget? Få en skræddersyet løsning' : 'Larger Budget? Get a Custom Solution'}</h3>
                                <p className="text-gray-400 mb-8 max-w-xl mx-auto">
                                    {isDa
                                        ? 'Vi administrerer budgetter fra 1.000 DKK til 100.000+ DKK. Kontakt os for en strategisk gennemgang.'
                                        : 'We manage budgets from 1,000 DKK to 100,000+ DKK. Contact us for a strategic review.'}
                                </p>

                                <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                                    <input
                                        type="text"
                                        placeholder={isDa ? 'Dit månedlige budget' : 'Your monthly budget'}
                                        value={customBudget}
                                    ...
                                    />
                                    <button
                                        onClick={() => navigate('/contact?subject=marketing')}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-4xl font-bold text-[#1D1D1F] mb-4">
                            {isDa ? 'Din vej til 1. pladsen på 90 dage' : 'Your path to #1 in 90 days'}
                        </h2>
                        <p className="text-[#86868B] font-medium">
                            {isDa ? 'Vi følger en gennemtestet proces for at sikre de bedste resultater' : 'We follow a proven process to ensure the best results'}
                        </p>
                    </div>
            ...
            {/* CROSS-SELL TO LISTING */}
            <section className="py-24 bg-white border-t border-gray-100">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <h2 className="text-3xl font-bold text-[#1D1D1F] mb-6">
                        {isDa ? 'Ikke klar til fuld skalering endnu?' : 'Not ready for full scaling yet?'}
                    </h2>
                    <p className="text-[#86868B] text-lg mb-10 font-medium">
                        {isDa 
                            ? 'Start med at blive en del af vores marketplace. Opret din profil, bliv verificeret og bliv fundet af lokale kunder.' 
                            : 'Start by becoming part of our marketplace. Create your profile, get verified, and be found by local customers.'}
                    </p>
                    <button
                        onClick={() => navigate('/for-businesses')}
                        className="inline-flex items-center gap-2 px-8 py-4 bg-gray-50 text-[#1D1D1F] rounded-xl font-bold hover:bg-gray-100 transition-all border border-gray-200"
                    >
                        {isDa ? 'Se hvordan du bliver listet' : 'See how to get listed'}
                        <ArrowRight size={18} />
                    </button>
                </div>
            </section>
