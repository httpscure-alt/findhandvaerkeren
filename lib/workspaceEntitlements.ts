/**
 * Advero workspace feature flags — one platform, modular layers per package.
 * @see product spec: SEO_ENABLED, ADS_ENABLED, AI_VISIBILITY_ENABLED
 */

export type WorkspaceEntitlementKey = 'seo' | 'ads' | 'aiVisibility';

/** Product lines (not tier levels Starter/Growth/Pro). */
export type ProductPackageId = 'starter' | 'seo' | 'sem' | 'ai_visibility' | 'growth_plus';

export interface WorkspaceEntitlements {
  /** SEO_ENABLED — SEO modules, rescans, SEO ops queue */
  seo: boolean;
  /** ADS_ENABLED — Google Ads dashboard, Ads ops queue */
  ads: boolean;
  /** AI_VISIBILITY_ENABLED — AI-readiness, semantic / conversational workflows */
...
/** Map Stripe tier id + service line → feature flags. */
...
function entitlementsFromItems(items: SubscriptionItemRecord[]): WorkspaceEntitlements {
  let seo = false;
  let ads = false;
